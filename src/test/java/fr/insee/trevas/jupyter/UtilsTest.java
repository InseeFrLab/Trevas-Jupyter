	...
	void testURI() throws URISyntaxException {
		var bases =
				List.of(
						...
						"/foo/bar/file.csv");
		var queries = List.of("", "?sep=,&=del=;", "?sep=%2C&del=%3B");
		var fragments = List.of("", "#fragment");
		for (String base : bases) {
			for (String query : queries) {
				for (String fragment : fragments) {
					URI uri = URI.create(base + query + fragment);

					assertThat(Optional.ofNullable(uri.getQuery()).map(f -> "?" + f).orElse(""))
							.isEqualTo(URLDecoder.decode(query, StandardCharsets.UTF_8));
					assertThat(uri.getPath()).isEqualTo("/foo/bar/file.csv");
					assertThat(Optional.ofNullable(uri.getFragment()).map(f -> "#" + f).orElse(""))
							.isEqualTo(fragment);
					...
					System.out.printf(
							"Query: %s, Fragment: %s%n", uri.getQuery(), uri.getFragment());
				}
			}
		}
	}
